...
interface CompatibilitySearchParams {
  q?: string | string[];
...
function parseStatus(value: string | undefined): GameStatus | null {
  if (!value) return null;
  return STATUS_ORDER.includes(value as GameStatus) ? (value as GameStatus) : null;
}

function parsePerf(value: string | undefined): PerfTier | "all" {
  if (value === "great" || value === "ok" || value === "poor") return value;
  return "all";
}
...
export default async function CompatibilityPage({
  ...
  const deviceCount = new Set(allGames.map((game) => game.lastReport.device)).size;
